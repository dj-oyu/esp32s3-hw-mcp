...
**この節の2件は 2026-09-15 に実機で実施・計測済み。結果は §2.1〜§2.3 に追記した。**
...
### 2.2 装飾光線のプロファイルを support で門番する — 厳密だが、速くはならなかった（2026-09-15）

cardputer-adv-pocketjs の新ブランチ `perf/flower-decor`（親は design-contracts + SPLIT3）。
`garden_decor_profile()` は半径の外で厳密に 0 を返すので、light と shadow はそれぞれ台が有限で、ループが
歩くスパンは両者の和集合（最大 3.5 半径）。自分の到達範囲の外では評価しない門番を入れた。

- **厳密性**: 門番 ON / OFF の両方が変更前リビジョンとバイト一致（120 フレーム × 135 行 = 7.8 MB、
  `tools/flower_decor_identity.c`）。
- **ホストで数えた効果**: プロファイル評価が 4,304,780 → 2,772,750（**35.6% 減**、1 フレーム 12,767 回減）。
  命令数モデルでは ~0.7 ms の節約になる。
- **実機では測れなかった**: 1 バイナリ内で 60 フレーム窓ごとにスイッチを反転し（SPLIT3 に `gate=`）、
  隣接 41 ペアで差分を取ると rays 平均 **-0.19 ms**（中央値 -0.03、min -4.10、max +5.24）、
  **同じコードの vegetation を対照に置くと平均 -0.09 ms**（＝ノイズ床）。効果はノイズの中。

**結論（次の一手の根拠が変わった）**: 装飾光線パスは**プロファイル演算には律速していない**。3分の1を
消しても動かないので、per-pixel の費用は dither・pack・load/store とその周りのストール
（訪問画素あたり ~60〜150 サイクル vs 命令数 ~70）が占めている。**このループの算術を削る方向は打ち止めで、
`rays` を動かすには per-pixel パイプラインごとベクタ化（PIE）するしかない。**

**計測方法の教訓（この repo で再利用する）**: この規模の変更は「ビルド間比較」では判定できない
（コード配置で最大15%動く）。**同一バイナリ内で窓ごとにスイッチを反転させ、隣接窓のペアで差分を取る**のが
唯一使える方法で、対照（触っていないコード）を同時に出すとノイズ床が分かる。

### 2.3 `snprintf(meter, ...)` の移動
