...
> `ex14_raster.S` に a10..a15 の退避・復帰（prologue の `s32i` と各 `retw.n` 前の `l32i`）を追加したため、
> **関数全体のサイズ・命令数は増えている**（このファイルで 20 命令）。**ループ本体と 1 反復あたりの命令数は不変**（ループ内は無改変）。
> md5: `0dbb047adfba957b5dcaa602728bf1d7` → `4984610e735a940965f410c4a69984ae`（tools/check_abi.py / tools/fix_abi.py）
